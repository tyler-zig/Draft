    ...
    <div className="cc-table-wrap" ref={rootRef}><table className="cc-player-table"><thead><tr>{TABLE_COLUMNS.filter((column) => visibleColumns.has(column.key)).map((column) => <th key={column.key} className={column.compact ? 'cc-col-fit' : undefined}><button type="button" className="cc-sort-head" title={column.hint} onClick={() => changeSort(column.key)}>{column.label}{column.hint && !column.compact ? <i className="cc-hint" aria-hidden="true">ⓘ</i> : null}{sortKey === column.key ? <span>{sortDirection === 'asc' ? '▲' : '▼'}</span> : null}</button></th>)}</tr></thead><tbody>
      {pageRows.map((player, index) => {
        ...
        const cells: Record<TableColumnKey, React.ReactNode> = {
          ...
          position: <span className={`cc-pos cc-${player.position.toLowerCase()}`}>{player.position}</span>, team: <span className="cc-num"><TeamLabel team={player.team} fallback="FA" /></span>, tier: <span className={`cc-tier-pill ${tierColorClass(playerTier)}`}>Tier {playerTier}</span>, adp: <span className="cc-num">{player.adp != null ? player.adp.toFixed(1) : '—'}</span>, liveAdp: <span className="cc-num">{player.liveAdp != null ? player.liveAdp.toFixed(1) : '—'}</span>, liveAdp1d: <span className={`cc-num ${liveAdpChangeTone(player.liveAdpVsLastOne)}`}>{formatLiveAdpChange(player.liveAdpVsLastOne)}</span>, liveAdp7d: <span className={`cc-num ${liveAdpChangeTone(player.liveAdpVsLastSeven)}`}>{formatLiveAdpChange(player.liveAdpVsLastSeven)}</span>, projection: <span className="cc-num"><ProjectionHover value={projection} breakdown={player.projectionBreakdown} label="Projected points" /></span>, vorp: <span className={`cc-num ${player.vorp == null ? '' : player.vorp >= 0 ? 'cc-up' : 'cc-down'}`}>{player.vorp == null ? '—' : `${player.vorp >= 0 ? '+' : ''}${player.vorp.toFixed(1)}`}</span>, value: <span className={`cc-num ${(value ?? 0) >= 0 ? 'cc-up' : 'cc-down'}`}>{value == null ? '—' : `${value >= 0 ? '+' : ''}${value.toFixed(1)}`}</span>, sos: <span className={`cc-num cc-sos-${matchupTone(player.playoffSos?.rank)}`}>{player.playoffSos?.rank != null ? ordinal(player.playoffSos.rank) : '—'}</span>,
        }
        return <tr key={player.id} className={`${selectedId === player.id ? 'cc-selected' : ''} ${isTaken ? 'cc-taken' : ''}`} onMouseEnter={() => prefetch(player)} onFocus={() => prefetch(player)} onPointerDown={() => prefetch(player)} onClick={() => onSelect(player.id)} onDoubleClick={() => !isTaken && canDraft && onDraft?.(player.id)}>{TABLE_COLUMNS.filter((column) => visibleColumns.has(column.key)).map((column) => <td key={column.key} className={column.compact ? 'cc-col-fit' : undefined}>{cells[column.key]}</td>)}</tr>
      })}
